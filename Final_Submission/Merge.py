...
def combine_meta_data(df,meta_tuple,creative_tuple,meta_column,creat_col,option = 'both'):
  '''
  ...
  meta_column -> tuple of list ([features from iso],[features from andriod])
  creat_col -> a list of features from creative
  ...
  if (option == 'meta') | (option =='both'):
    ...
    meta_iso.columns=['market_id','title','created_at','last_updated_at',\
                      ...
                      'genres','user_ratings_current',\
                      'n_user_ratings_current','user_rating',\
    ...
    if 'market_id' not in meta_column[0]:
        meta_column[0].append('market_id') 
    if 'market_id' not in meta_column[1]:
        meta_column[1].append('market_id') 
    a = pd.merge(application,meta_iso[meta_column[0]],on='market_id',how='left')
    a = pd.merge(a,meta_android[meta_column[1]],on='market_id',how='left',suffixes=['','_android'])
  ...
  if (option == 'creative') | (option == 'both'): 
    ...
    '''
    if 'creative_id' not in creat_col:
        creat_col.append('creative_id')
    b= pd.merge(b, df_creative[creat_col], how='left', on='creative_id',suffixes=['','_creative'])
