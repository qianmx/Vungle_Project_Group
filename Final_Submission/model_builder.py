...
def logistic_model_final(X_train, y_train,c):
    """
    use crossvalidation (CV) to report the best parameter 'C'
    parameter C: Inverse of regularization strength; must be a positive float. 
    Check LogisticRegression() in sklearn for more information
...
def preprocessing_data(dff,numerical_list,string_list):
    """
    ...
    print 'filled na'
    df = convert_to_unknown(df,string_list) 
    print 'converted to unknown'
    #df = convert_to_string(df,numerical_list)
...
def convert_to_unknown(df, columns):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        df[columns] = df[columns].fillna("UNK")
